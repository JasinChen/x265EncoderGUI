...
class VS:
    ...
    def denoise(self, **args):
        if args:
            self.src = core.knlm.KNLMeansCL(core.knlm.KNLMeansCL(self.src, args), args)
        else:
    ...
    def ivtc(self, vd=1):
        ...
        matched_clip = core.vivtc.VFM(self.src, 1)
        deinterlaced_clip = core.tdm.TDeintMod(matched_clip, order=1,
                                               edeint=core.nnedi3cl.NNEDI3CL(matched_clip, field=1))
        combProps = core.tdm.IsCombed(matched_clip)
        postprocessed_clip = core.std.FrameEval(matched_clip, functools.partial(postprocess, clip=matched_clip,
                                                                                deinterlaced=deinterlaced_clip),
                                                combProps)
        if vd == 1:
