...
def CSMOD(filtered, source=None, pclip=None, chroma=None, preset=None, edgemode=None, edgemask=None,
    ...
    def CSmod_nrSpline64Resize(input, target_width=None, target_height=None, chroma=True, nr=nr, ss_wf=ss_w, ss_hf=ss_h):
        w = input.width
        h = input.height
        sbitPS = input.format.bits_per_sample
    ...
    def CSmod_nnedi3_SuperSample(src, ss_wf=ss_w, ss_hf=ss_h):
        # get cycle times
        cyclew = math.ceil(math.log(ss_wf, 2))
        cycleh = math.ceil(math.log(ss_hf, 2))
        # constant values
        sbitPS = src.format.bits_per_sample
        wss = round(src.width * ss_wf / 8) * 8
        hss = round(src.height * ss_hf / 8) * 8
        # nothing to say..
    ...
    def average(clipa, clipb, planes=None):
        if planes is None:
            planes = [0, 1, 2]
        if 1 or 2 in planes:
    ...
    def clamp(clip, Bclip, Dclip, overshoot=0, undershoot=0, planes=None):
        if planes is None:
            planes = [0, 1, 2]
        mt = 'x y {overshoot} + > y {overshoot} + x ? z {undershoot} - < z {undershoot} - x y {overshoot} + > y {overshoot} + x ? ?'.format(
        ...
        if 1 or 2 in planes:
            ...
            if not GRAYS:
                output = core.std.Expr([clip, Dclip, Dclip], [mt, ""])
            else:
                output = core.std.Expr([clip, Bclip, Dclip], [mt])
            return output
