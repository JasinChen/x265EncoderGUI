...
class internal:
      ...
      def basic(core, src, super, radius, pel, sad, short_time, color):
          ...
          if short_time:
             ...
             me_sad           += [sad]
             vmulti            = core.MAnalyze(supersoft, chroma=color, overlap=4, blksize=8, **manalyze_args)
             ...
             me_sad            = constant * math.pow(sad, 2.0) * math.log(1.0 + 1.0 / (constant * sad))
             vmulti            = core.MAnalyze(supersoft, chroma=color, overlap=64, blksize=128, **manalyze_args)
