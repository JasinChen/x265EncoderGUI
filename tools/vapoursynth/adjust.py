...
def Tweak(clip, hue=None, sat=None, bright=None, cont=None, coring=True):
    ...
    if (hue is not None or sat is not None) and clip.format.color_family != vs.GRAY:
        ...
        if coring:
            chroma_min = 16 << depth - 8
            chroma_max = 240 << depth - 8
        else:
            chroma_min = 0
            chroma_max = (1 << depth) - 1
    ...
    if bright is not None or cont is not None:
        ...
        if clip.format.sample_type == vs.INTEGER:
            if coring:
                luma_min = 16 << depth - 8
                luma_max = 235 << depth - 8
            else:
                luma_min = 0
                luma_max = (1 << depth) - 1

            clip = clip.std.Lut(planes=0,
                                function=functools.partial(lut, cont=cont, bright=bright, luma_min=luma_min, luma_max=luma_max))
        else:
...
def lut(x: int, cont: int, bright: int, luma_min: int, luma_max: int):
    val = int((x - luma_min) * cont + bright + luma_min + 0.5)
    return min(max(val, luma_min), luma_max)
